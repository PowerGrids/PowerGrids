...
model System "System object"
  ...
  parameter Boolean enableAssertions = true "=true, enables asserts in order to check the validity of the numerical solution" annotation(
    Evaluate = true, Dialog(group="Solution checking"));
  parameter AssertionLevel assertionLevel = AssertionLevel.warning "assertion level for the solution checking (AssertionLevel.error stops the simulation)" annotation(
  Evaluate = true, Dialog(group="Solution checking", enable=enableAssertions));
  parameter SI.PerUnit VPuMin = 0.7 "min value of voltage in p.u at buses and sinchronous generators nodes" annotation(
    Dialog(group="Solution checking"));
  parameter SI.PerUnit VPuMax = 1.3 "max value of voltage in p.u at buses and sinchronous generators nodes" annotation(
    Dialog(group="Solution checking"));
  parameter SI.PerUnit IPuMax = 2.5 "max value of current in p.u that flows through the loads" annotation(
    Dialog(group="Solution checking"));
  parameter SI.PerUnit omegaPuMin = 0.9 "min value of frequency in p.u at generators nodes" annotation(
    Dialog(group="Solution checking"));
  parameter SI.PerUnit omegaPuMax = 1.1 "max value of frequency in p.u at generators nodes" annotation(
    Dialog(group="Solution checking"));
  parameter Boolean computePF = true "= true, computes the start value with the embedded power flow" annotation(
