...
model SynchronousMachine4WindingsPF2
  ...
  Machines.SynchronousMachine4Windings machine3(H = 6, SNom = 5e8, Tpd0 = 8, Tppd0 = 0.03, Tppq0 = 0.07, Tpq0 = 1, UNom = 24000, excitationPuType = PowerGrids.Types.Choices.ExcitationPuType.Kundur, raPu = 0.003, timeConstApprox = PowerGrids.Types.Choices.TimeConstantsApproximation.classicalDefinition, xdPu = 1.81, xlPu = 0.15, xpdPu = 0.3, xppdPu = 0.23, xppqPu = 0.25, xpqPu = 0.65, xqPu = 1.76, redeclare model ComponentPF = PowerGrids.Electrical.PowerFlow.SlackBus(UNom = 24000, SNom = 5e8, setPhase = false) "slack bus is used in EPF instead of the PVBus, please manually fill the relevant parameters", useEPFtoSetExternalOffsetPQ = true) annotation(
...
<p>The adopted EPF strategy is to move the <i>EPF slack</i> node to the same node of the generator (by redeclaring its EPF component as a <a href=\"modelica://PowerGrids.Electrical.PowerFlow.SlackBus\">SlackBus</a>). Moreover, in order to maintain the same <i>reference node</i> in both the EPF and the <i>dynamic</i> model, the <i>EPF slack</i> node doesn't prescribe the <i>reference angle</i> (flag <code>setPhase = false</code> in the redeclaration of the EPF component in the <i>generator</i>), the <i>reference angle</i> is still prescribed by the EPF component of the <i>reference node</i> (<a href=\"modelica://PowerGrids.Electrical.PowerFlow.PhaseReferenceBusPF\">PhaseReferenceBusPF</a>, redeclared as EPF component in the <i>reference node</i>).</p>

<p>The <i>generator</i> has no <i>control</i> so two <a href=\"modelica://PowerGrids.Controls.FreeOffset\"><i>free offset</i></a> components have been used in order to calculate the values of the <code>PmPu</code> and the <code>ufPuIn</code> inputs that maintain the solution calculated by the EPF. The values of the <b><i>offset</i></b> must be back-calculated by the <i>dynamic</i> initialization, starting from the solution calculated by the EPF. To do this two additional initial equations must be provided, in this example that equations are provided by the generator, by setting the flag <code>useEPFtoSetExternalOffsetPQ = true</code>, so that the the generator is enforced to supply the same <code>P</code> and <code>Q</code> calculated by the <i>EPF slack</i></p>
...
<p>It is possible to see that the working point calculated by the dynamic initialization is exactly the same calculated by the EPF, independently from the type of the load (<i>constant impedance</i> or <i>constant power</i>). This becuse the EPF strategy has maintained the same <i>reference node</i> of the dynamic initialization.</p>
