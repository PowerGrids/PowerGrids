...
<p>The adopted EPF strategy is to move the <i>EPF slack</i> node to the same node of the generator (by redeclaring its EPF component as a <a href=\"modelica://PowerGrids.Electrical.PowerFlow.SlackBus\">SlackBus</a>) and to set the generator flag <code>useEPFtoSetExternalOffsetPQ = true</code> so that the the generator is initialized by the <code>P</code> and the <code>Q</code> calculated by the <i>EPF slack</i>.</p>
