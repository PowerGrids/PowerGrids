...
<p> Note that both the active and reactive power injected by the reference node are null (as required) but the working point of the load - in terms of active and reactive power - is slightly different from the one used to calculate the EPF (<b>500MW</b> and <b>300MVAR</b>), this because the dynamic model of the load is of constant impedance type, so its working point depends on both the voltage and the phase at its node, while the EPF model of the load prescribes both the active and reactive power independently from the voltage and the phase at the same node. This model difference slightly moves the working point during the dynamic initialisation.</p>
<p>The numerical result is quite the same in the two cases, this because both the voltage and phase at the generator (slack) node in the second case have been set as near as possible to the result of the first case. Generally the result could be different in the two cases, because the EPF is structurally different, so that also the initialization of the dynamic model is generally different and thus it could converge to a different initial working point.</p>
</body></html>"),
