...
<p>It shows two different ways to set the position of the slack node: the first (blue box) is the default behavior of the Embedded Power Flow (EPF) that uses the reference bus as slack node (S is shown in the icon) and the generator as a PVBus; the second (green and red boxes) redeclares the EPF components in the reference bus and the generator in order to move the slack bus to the generator node (S is shown in the generator icon), in this case the reference bus is redeclared as a generic bus in the EPF (note that it still works as a <a href=\"modelica://PowerGrids.Electrical.Buses.ReferenceBus\">reference bus</a> during both the dynamic initialization and the simulation phases).
</p><p>Herebelow the simulation result (do not consider the model in the red box for the moment):</p><p></p><figure>
...
<p>The numerical results are very near in the two cases, because both voltage and phase at the generator (slack) node in the second case has been set to be equal to the result of the first one (and actually is very near to it). Generally, the differences could be larger, because the initialization of the dynamic model is&nbsp;structurally&nbsp;different from the EPF's.</p>
<p>The example shows also a third model (red box) which is a variation of the second one (green box): in this case, the slack node is still in the generator, but the load is constant power (as in the PF). Therefore the power absorbed by the load stays wall to the one in the first case (blue box).</p>
</body></html>"),
