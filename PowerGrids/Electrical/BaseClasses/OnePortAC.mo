...
partial model OnePortAC "Base class for AC components with one port"
  ...
  PowerGrids.Interfaces.PowerTerminalAC powerTerminal
...
equation
  if initial() and localInit == LocalInitializationOption.PV then
    ...
    CM.real(powerTerminal.v*CM.conj(powerTerminal.i)) = port.PStart;
    CM.'abs'(powerTerminal.v) = port.VStart;
    port.Q = port.QStart;
  ...
  elseif initial() and localInit == LocalInitializationOption.PQ then
    ...
    powerTerminal.v*CM.conj(powerTerminal.i) = Complex(port.PStart, port.QStart);
    port.v.re*port.vStart.im = port.v.im*port.vStart.re;
    ...
    port.v = powerTerminal.v;
    port.i = powerTerminal.i;
  end if;
  annotation(
    Documentation(info = "<html>
<p>This is the base class for all the components with an AC power terminal. It contains a corresponding <code>PortAC</code> component to compute useful quantities for modelling and monitoring purposes.</p>
</html>"));
end OnePortAC;
