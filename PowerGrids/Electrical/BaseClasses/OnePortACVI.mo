...
partial model OnePortACVI "Base class for naked AC components with one port"
  ...
  parameter Types.ApparentPower SNom(start = 100e6) "Nominal/rated apparent power, also used as p.u. base" annotation(
    Evaluate = true);
  parameter Types.Voltage UStart = if computePF or isOnePortAC then UStartPF else UNom "Start value of phase-to-phase voltage modulus";
  parameter Types.Angle UPhaseStart = if computePF or isOnePortAC then UPhaseStartPF else 0 "Start value of voltage phase";
  ...
  parameter Boolean isOnePortACBus = false "= true for OnePortACBus models";
