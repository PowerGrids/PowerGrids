...
protected
    ...
    Boolean valueInRange = not(valueUnderMin) and not(valueAboveMax) "True if the monitored signal is into the [valueMin ; valueMax] range";
...
equation
  lookingToDecreaseTap = (running and valueAboveMax and directAction) or (running and valueUnderMin and reverseAction);
  lookingToIncreaseTap = (running and valueUnderMin and directAction) or (running and valueAboveMax and reverseAction);

  when running and not(locked) and valueUnderMin then
  ...
  elsewhen running and not(locked) and not(valueUnderMin) then
    tValueUnderMinWhileRunning = pre(tValueUnderMinWhileRunning);
    tValueAboveMaxWhileRunning = pre(tValueAboveMaxWhileRunning);
    tValueInRangeWhileRunning = time;
  elsewhen running and not(locked) and valueAboveMax then
    tValueUnderMinWhileRunning = pre(tValueUnderMinWhileRunning);
    tValueAboveMaxWhileRunning = time;
    tValueInRangeWhileRunning = pre(tValueInRangeWhileRunning);
  elsewhen running and not(locked) and not(valueAboveMax) then
    tValueUnderMinWhileRunning = pre(tValueUnderMinWhileRunning);
    tValueAboveMaxWhileRunning = pre(tValueAboveMaxWhileRunning);
    tValueInRangeWhileRunning = time;
  ...
  elsewhen running and pre(state) == State.locked and not(locked)
                   and valueInRange 
