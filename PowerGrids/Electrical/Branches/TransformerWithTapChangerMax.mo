...
// Tap changer is applied on output phase-to-phase voltage module
  ...
  Documentation(info = "<html><head></head><body>This model implements a transformer with<span style=\"font-size: 12px;\">&nbsp;a tap changer which takes a maximum and stop phase-to-phase voltage magnitude limits, and tries to bring the magnitude of the phase-to-phase voltage at portB back to the stop limit when the maximum is reached.</span><div><br></div><div><div><span style=\"font-size: 12px;\">It extends two classes:</span></div><div><div style=\"font-size: 12px;\"><ul><li><a href=\"Modelica:///PowerGrids.Electrical.Branches.BaseClasses.TapChangerPhaseShifterCommon\">TapChangerPhaseShifterCommon</a>, which implements the model of the transformer,</li></ul><ul><li><a href=\"Modelica:///PowerGrids.Electrical.Branches.BaseClasses.TapChangerPhaseShifterLogicMax\">TapChangerPhaseShifterLogicMax</a>, which implements the logic that updates the position of the tap changer.</li></ul><div><br></div></div></div></div></body></html>"));
