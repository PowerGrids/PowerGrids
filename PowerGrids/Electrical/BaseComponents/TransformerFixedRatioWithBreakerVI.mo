...
partial model TransformerFixedRatioWithBreakerVI
...
equation
  ...
  if not useBreaker then
    breakerStatusInInternal = breakerStatusStart "Breaker initial status";
  end if;
...
<p>Implemented as a child class of <a href=\"modelica://PowerGrids.Electrical.BaseComponents.PiNetworkVI\">PiNetworkVI</a>, where Ya=0 Yb=G+jB and k = rFixed*exp(j*thetaFixed), see the corresponding documentation.</p><p>One breaker is provided at port b side. The breaker has a conditional boolean input to set the logical status:<br></p><ul><li>true &nbsp;= breaker closed</li><li>false = breaker open&nbsp;</li></ul>
<p>When the breaker input is not used the status of the breaker is assigned by using the initial status parameter.</p>
<div>When the breaker is open the current entering from the port b is forced to 0 (by setting to 0 both the actual values of the series impedance and of the shunt impedance of port b), and the total impedance seen to the side a (high-level voltage side) becomes equal to the line impedance + the shunt impedance.</div><div><br></div><div>The actual values of the series impedance and of both the shunt impedances are managed by a state-machine, which monitors the state of the breaker.</div>
</body></html>"));
end TransformerFixedRatioWithBreakerVI;
