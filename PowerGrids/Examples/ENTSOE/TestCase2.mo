...
model TestCase2 "Test Case 2, Section 5.2, focuses on the dynamic behavior of the model for the synchronous generator and its governor"
  ...
  Modelica.Blocks.Sources.RealExpression VrefPu(y = 1.00943) annotation(
    Placement(transformation(origin = {-62, -63}, extent = {{-12, -11}, {12, 11}})));
  Modelica.Blocks.Sources.RealExpression RefLPu(y = 380/475*0.05) annotation(
    Placement(transformation(origin = {-52, 12}, extent = {{-12, -12}, {12, 12}})));
  ...
  Electrical.Buses.Bus NGEN(UNom = 21000)  annotation(
    Placement(transformation(origin = {30, -16}, extent = {{-10, -10}, {10, 10}})));
  Controls.FreeOffset freeOffsetAFR(use_u = true)  annotation(
    Placement(transformation(origin = {-23, -63}, extent = {{-7, -7}, {7, 7}})));
  Controls.FreeOffset freeOffsetGOV(use_u = true)  annotation(
    Placement(transformation(origin = {-24, 12}, extent = {{-6, -6}, {6, 6}})));
equation
  connect(GEN.VPu, AVR.VcPu) annotation(
    Line(points = {{36, 1}, {44, 1}, {44, 34}, {-72, 34}, {-72, -20}, {-30, -20}}, color = {0, 0, 127}));
  ...
  connect(AVR.efdPu, GEN.ufPuIn) annotation(
    Line(points = {{-9, -22}, {-2, -22}, {-2, 2}, {24, 2}}, color = {0, 0, 127}));
  connect(GEN.omegaPu, TGOV.omegaPu) annotation(
    Line(points = {{36, -3}, {42, -3}, {42, 30}, {-14, 30}, {-14, 20}, {-8, 20}}, color = {0, 0, 127}));
  connect(TGOV.PMechPu, GEN.PmPu) annotation(
    Line(points = {{14, 16}, {18, 16}, {18, -4}, {24, -4}}, color = {0, 0, 127}));
  ...
  connect(NGEN.terminalAC, LOAD.terminalAC) annotation(
    Line(points = {{30, -16}, {30, -24}, {68, -24}, {68, -44}}));
  connect(VrefPu.y, freeOffsetAFR.u) annotation(
    Line(points = {{-49, -63}, {-31, -63}}, color = {0, 0, 127}));
  connect(AVR.VrefPu, freeOffsetAFR.y) annotation(
    Line(points = {{-30, -24}, {-34, -24}, {-34, -34}, {-10, -34}, {-10, -63}, {-15, -63}}, color = {0, 0, 127}));
  connect(RefLPu.y, freeOffsetGOV.u) annotation(
    Line(points = {{-39, 12}, {-31, 12}}, color = {0, 0, 127}));
  connect(freeOffsetGOV.y, TGOV.RefLPu) annotation(
    Line(points = {{-17, 12}, {-8, 12}}, color = {0, 0, 127}));
  annotation(
